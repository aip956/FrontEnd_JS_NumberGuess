...
function submit_guess(guess) {
    ...
    if (is_val_guess != 1) {
        ...
        get_misplaced(secr_count_array, guess_count_array, well_placed)
    }
}
...
function play_round() {
    ...
    while (round_num <= max_tries && you_won == 0) {

        console.log("Round #", round_num)
        const prompt = require('prompt-sync')();
        let guess = prompt("Please enter a 4 digit guess: ")
        // document.getElementById("round-num").innerHTML = round_num;
            round_complete = 0;
...
// Play the game
function play_game(secret, secr_count_array) {
    // round_num = 1; 
    // tries = 10
    make_secret()
    secret_counts()
